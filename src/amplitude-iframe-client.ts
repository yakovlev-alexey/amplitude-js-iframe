...
import type { MessageType, Instances, MessagePort, PortMessageType, MessagePayload } from "./types";
...
export class AmplitudeIframeClient {
    ...
    public static getInstance(id: string) {
        const instanceId =
            typeof id === "string" && Boolean(id) ? id.toLowerCase() : DEFAULT_INSTANCE_ID;
        ...
        return (this._instances[instanceId] = new AmplitudeIframeClient(instanceId));
    }

    public init(port: MessagePort, apiKey: string, userId?: string, config?: Config) {
    ...
    private _waitForInitialization(apiKey: string, userId?: string, config?: Config) {
        ...
        const handleInitialized = (event: MessageEvent<{ type: PortMessageType }>) => {
    ...
    private _clearQueue() {
        while (this._queue.length) {
            // length is checked synchronously
